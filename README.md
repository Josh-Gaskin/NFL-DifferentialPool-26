...
- **Firebase Spark plan** (free, no card required) stores your locked-in picks so you and your partner both see live updates. No Cloud Functions are used (Spark doesn't include them), so everything here works without ever touching the paid Blaze plan.

There's no sign-in — the site is open to anyone with the link (see the security note below). Simpler to set up and use, at the cost of no real access control.

## 1. Firebase setup

1. Go to [console.firebase.google.com](https://console.firebase.google.com) → **Add project** (free, no billing needed for what we use).
2. **Build → Firestore Database → Create database → Start in production mode** (any region near you).
3. **Project settings (gear icon) → General → Your apps → Web (`</>`)** → register an app (no Hosting needed) → copy the `firebaseConfig` object.
4. Paste those values into `js/firebase-config.js`, replacing the placeholders.
5. Deploy the security rules. Easiest way, no local install required: in the Firestore Database page, click **Rules** tab, paste the contents of `firestore.rules`, and click **Publish**.
...
Because there's no sign-in, anyone with your site's URL can read and write your picks (the Firestore rules allow open access, and the Firebase config is visible in the page source either way). For a private pool between you and your partner this is a reasonable tradeoff — just don't post the link somewhere public. If you'd rather have real access control, let me know and I can add Google sign-in back, restricted to your two accounts.
